...
class QueryZen:
    ...
    def create(
        ...
        if not response.data:
            raise UnknownError('When creating a Zen, the JSON representation of the object was not returned') # Fixme improve this
    ...
    def list(self, **filters) -> list[Zen]:
        ...
        It does not raise Exceptions, if you want to be extra safe, use QueryZen.get or QueryZen.get_or_create
        ...
        Examples:
            ...
            # Will return all ``Zen``s with name 'mountain' whose execution status is valid # todo check wording on this
    ...
    def get_or_create(
        ...
        Examples:
            >>> created, zen = QueryZen().get_or_create(name='myzen', query='select * from t where h > :value')
        ...
        Args:
            ...
            collection: The collection of the Zen (defaults to ``DEFAULT_COLLECTION``).
            version: The version of the Zen (defaults to 'AUTO', the latest will be returned if it exists).
